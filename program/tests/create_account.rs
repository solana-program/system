...
use {
    ...
    solana_system_interface::{
        error::SystemError, instruction::create_account, MAX_PERMITTED_DATA_LENGTH,
    },
};
...
fn fail_to_has_lamports() {
    ...
    mollusk.process_and_validate_instruction(
        ...
        &[(from_pubkey, Account::default()), (to_pubkey, to_account)],
...
#[test]
fn fail_to_not_signer() {
    let mollusk = setup::setup();

    let from_pubkey = Pubkey::new_unique();
    let to_pubkey = Pubkey::new_unique();

    let mut instruction = create_account(&from_pubkey, &to_pubkey, LAMPORTS, SPACE, &OWNER);
