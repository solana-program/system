...
it('infers the resolved account metas from the direct getter inputs', async () => {
    ...
    // source collapses to a writable signer meta, while a non-signing address carrier or a
    // program-derived address destination collapses to a plain writable account keyed by its
    // branded address — with no signer attached.
    const source = await generateKeyPairSigner();
    ...
    // The signer source resolves to a signer meta carrying the attached signer and the source's
    // branded address, whereas the non-signing PDA destination resolves to a plain account meta
    // keyed by the PDA's branded address with no `signer` property attached.
