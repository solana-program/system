...
// TODO: Replace this with the pubkey! macro.
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self {
            SystemError::AccountAlreadyInUse => {
                write!(f, "an account with the same address already exists")
            }
            ...
            SystemError::InvalidProgramId => {
                write!(f, "cannot assign account to this program id")
            }
            SystemError::InvalidAccountDataLength => {
                write!(f, "cannot allocate account data of this length")
            }
            SystemError::MaxSeedLengthExceeded => {
                write!(f, "length of requested seed is too long")
            }
            ...
            SystemError::NonceBlockhashNotExpired => {
                write!(f, "stored nonce is still in recent_blockhashes")
            }
            SystemError::NonceUnexpectedBlockhashValue => {
                write!(f, "specified nonce does not match stored nonce")
            }
        }
    }
}
